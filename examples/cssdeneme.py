...
from ait_ui.elements import Element, Text, Image, TextArea , Button , Elm
...
class CssDeneme():
            ...
            with Element().cls("container").style("padding-bottom","10px").style("height", "100vh").style("flex-direction", "column").style("gap", "10px"):
                ...
                with Element().cls("container").style("flex-direction", "row").style("gap", "10px"):
                    with Element().cls("wrapper").style("width", "1410px").style("gap", "10px").style("height", "100%").style("flex-direction", "row"):
                        ...
                        with Element().cls("wrapper").style("flex-direction", "column").style("gap", "10px"):
                            Button(value="Run").cls("btn btn-green").on("click", self.on_click)
                ...
                with Element().cls("container").style("flex-direction", "row").style("gap", "10px").style("height", "100%"):
                    with Element().cls("wrapper").style("width", "700px").style("gap", "10px").style("height", "100%").style("flex-direction", "column"):
                        Comp_InputFile(id="dropzone").on("input", self.setImage)
                        with Element(id="accordion").cls("accordion").style("min-height", "65px"):
                            with Button(id="accordion-button").on("click", self.change_accordion):
                            ...
                            with Element(id="accordion-wrapper"):
                                with Element().cls("accordion-element-wrapper"):
                                    with Element().cls("slider-wrapper"):
                                        Comp_Slider(label="Slider 1", id="slider1", min=0, step=1 , max=100, value=50).on("change_slider", self.change_slider_value)
                                    with Element().cls("slider-wrapper"):
                                        Comp_Slider(label="Slider 2", id="slider2", min=0,step=1 , max=100, value=50).on("change_slider", self.change_slider_value)
                                with Element().cls("accordion-element-wrapper"):
                                    with Element().cls("slider-wrapper"):
                                        Comp_Slider(label="Slider 3", id="slider3", step=5 , min=0, max=100, value=50).on("change_slider", self.change_slider_value)
                                    with Element().cls("slider-wrapper"):
                                        pass
                                Comp_Select(id="select1",options=self.options).on("change", self.on_change)
    ...
    #await function
    def setImage(self, id, value):
        image = Elm(id="image1")
        image.value = value
        print(value)

    def change_slider_value(self, id, value):
        print(id, value)
...
if __name__ == "__main__":
    app.run(ui = CssDeneme, debug=True)